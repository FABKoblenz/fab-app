...
from models.models import FABCartItem, FABOrder, FABOrderItem, FABOrderWithItems, FABItem
...
def get_order_details(order_id: int, commons: CommonDeps = Depends(common_deps)) -> FABOrderWithItems:
    ...
    for order, item in result:
        ...
        items.append(FABOrderItem(**item.model_dump()))
    return FABOrderWithItems(user_id=user_id, timestamp=timestamp, total_price=total_price, items=items)


@router.get("/")
async def get_orders(commons: CommonDeps = Depends(common_deps)):
    pass
